...
export default async function LeadsPage(){
  ...
  return <>
    <section className="hero">
      <h1>Clients & Leads</h1>
      <p className="muted">Create and track inquiries, tours, quotes, contracts, and bookings.</p>
    </section>
    ...
    <div className="grid">
      <section className="card span-4">
        ...
        <form action={createLead} className="form-grid">
          ...
          <div className="full"><label>Event Type</label>
            <select name="event_type">
              <option>Wedding</option><option>Corporate Event</option><option>Private Event</option>
              <option>Nonprofit Event</option><option>Other</option>
            </select>
          </div>
          ...
          <div><label>Source</label>
            <select name="source">
              ...
              <option>Phone</option><option>Walk-in</option><option>Wedding Show</option><option>Other</option>
            </select>
          </div>
          <div><label>Status</label>
            <select name="status">
              <option value="inquiry">Inquiry</option><option value="toured">Toured</option>
              <option value="quoted">Quoted</option><option value="contracted">Contracted</option>
      ...
      <section className="card span-8" style={{overflowX:"auto"}}>
        <h2>Shared Lead Records</h2>
        <table>
          <thead><tr><th>Client</th><th>Event</th><th>Date</th><th>Guests</th><th>Status</th><th>Follow-up</th><th>Assigned</th></tr></thead>
          <tbody>
            {(leads??[]).map(l=><tr key={l.id}>
              <td><b>{l.name}</b><br/><span className="muted">{l.email||l.phone||""}</span></td>
              <td>{l.event_type}</td>
              <td>{l.event_date||"Not set"}</td>
              <td>{l.guests||""}</td>
              <td><span className="badge">{l.status}</span></td>
              <td>{l.follow_up_date||"Not set"}</td>
              <td>{l.assigned_staff||"Unassigned"}</td>
            </tr>)}
          </tbody>
        </table>
      </section>
